...
namespace DDG
{
   ...
   // A must be symmetric; x is used as an initial guess
   {
      ...
      for( int iter = 0; iter < maxEigIter; iter++ )
      {
        ...
        if( ignoreConstantVector ) {
            // remove constant component by subtracting mean
            x.removeMean();
        }
        // normalize x
        x.normalize();
   ...
   // A and B must be symmetric; x is used as an initial guess
   {
      ...
      // Precalculate normalized 1 matrix
      DenseMatrix<T> e(x.nRows(), 1);
      e.zero(1);
      e /= std::sqrt((e.transpose()*(B*e))(0).re);
      
      // Start with a random x
      x.randomize();
      
      // B gives the norm and is used for 
      ...
      for( int iter = 0; iter < maxEigIter; iter++ )
      {
        ...
        // solve A x_i = B x_{i-1}
        DenseMatrix<T> Bx = B*x;
        solve(A, x, Bx);

        // remove constant component by subtracting mean
        x -= (x.transpose()*(B*e))(0)*e;

        // normalize x
        x /= std::sqrt((x.transpose()*(B*x))(0).re);
      }
      int t1 = clock();
   ...
   // A must be positive (semi-)definite; x is used as an initial guess
   {
      ...
      // Create a SparseFactor from A
      SparseFactor<Complex> L;
      L.build(A);
      ...
      {
        // solve A x_i = x_{i-1}
        // using backsolvePositiveDefinite() instead of solve()
   ...
   // A must be positive (semi-)definite; x is used as an initial guess
   {
      ...
      // same thing as above but this time with an inner product given by B
      ...
      for( int iter = 0; iter < maxEigIter; iter++ )
      {
        ...
        // using backsolvePositiveDefinite() instead of solve()
        DenseMatrix<T> Bx = B*x;
        backsolvePositiveDefinite(L, x, Bx);
   ...
   {    
     return (A*x - (x.transpose() * (A * x))(0)*x).norm() / x.norm();
   }
   ...
   {
     return (A*x - (x.transpose() * (A * x))(0)*x).norm() / std::sqrt((x.transpose()*(B*x))(0).re);
   }
